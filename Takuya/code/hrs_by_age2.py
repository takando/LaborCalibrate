...
def hrs_by_age(beg_mmyy, end_mmyy, web=False, directory=None, graph=False,
               graph_type='bokeh', age_bins=None, l_tilde=1, val_type="avg"):
    '''
    --------------------------------------------------------------------
    ...
    df_hrs_age = recalculate_avg_hours(file_paths, age_bins, val_type)
...
def recalculate_avg_hours(file_paths, age_bins, val_type):
    '''
    --------------------------------------------------------------------
    ...
    # group according to age
        ...
        df_hrs_age = np.max(np.where(df_hrs_age <= 168, df_hrs_age, 168))
...
def fit_cbsv(hrs_data, age_bins, graph_type, degree):
    cbsv = np.polynomial.chebyshev.Chebyshev.fit(age_bins, hrs_data, deg=degree)
    fit_data = cbsv(age_bins)
    ...
    if graph_type =="plt":
        ...
        for i in range(len(age_bins)-1):
            ...
            num_age_bins.append(i)
        fig, ax = plt.subplots()
        plt.plot(num_age_bins, hrs_data, label='Average hours by age')
        plt.plot(num_age_bins, fit_data, color="r")
        plt.xticks(num_age_bins, age_pers)


def cheb_polynom(hrs_data, age_bins, degree):
    cbsv = np.polynomial.chebyshev.Chebyshev.fit(age_bins, hrs_data, deg=degree)
    unsc_coef = cbsv.convert().coef
    cb_coef = P.chebyshev.cheb2poly(unsc_coef)
    
    val = sympy.Symbol("age")

    cb_cov = np.array([1])
    for i in range(degree):
        cb_cov = np.append(cb_cov, val**(i+1))

    cheb_polynom = np.sum(np.multiply(cb_coef, cb_cov))
...
def adj_cbsv(hrs_data, age_bins, age_patch, age_ub,degree, graph="plt"):
    cbsv = np.polynomial.chebyshev.Chebyshev.fit(age_bins, hrs_data, deg=degree)
    fit_data = cbsv(age_bins)

    cheb_polyn = cheb_polynom(hrs_data, age_bins, degree)

    patch_bins = np.arange(age_patch, age_ub +1)

    patch_values = fit_curv(cheb_polyn, age_patch, age_ub)

    loc_patch = len(age_bins[age_bins<=age_patch])

    act_data = fit_data[:loc_patch]

    ext_data = np.append(act_data, patch_values[1:])

    ext_bins = np.append(age_bins[:-1], patch_bins)
    ...
    if graph=="plt":
        ...
        fig, ax = plt.subplots()
        plt.plot(num_age_bins, hrs_data, label='Average hours by age')
        plt.plot(num_age_bins, ext_data[:len(hrs_data)], color="r")
        plt.xticks(num_age_bins, age_pers, rotation=60)
